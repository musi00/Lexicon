...
#define BaicLexiconTestEnabled  1
...
void BasicLexiconTest() try {
  ...
  for (string& w : words) {
    ...
    CheckCondition(lex.containsPrefix(w.substr(0,3)), "Lexicon contains a 
        prefix of the " + word + "word");
  }

  /* Make sure different length prefixes can be found */
  for (int& i: {1,2,3,5}) {
    string prefix = word[0].substr(0,i);
  ...
  CheckCondition(lex.size() == 2,     "Lexicon reporting correct size after 
      removal");
  set<string> lex_set;
  bool correct_contents = lex_set[1] == "second" && lex_set[1] == "third";
  CheckCondtion(correct_contents, "Correct lexicon contents after remove");
  ...
  CheckCondition(lex.size() == 3, "Lexicon has correct size");
  set<string> lex_set = lex.toSTLSet();
  correct_contents = lex_set[0] == "second" && lex_set[1] == "st" && 
      lex_set[2] == "third";
